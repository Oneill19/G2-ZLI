...
public class OrderManagementScreenController {
	...
	void OnAprrove(ActionEvent event) throws Exception {
		...
		if (Selectedorder == null)
			...
			if ((StatusOrder.equals("WAITING_FOR_CANCELATION"))
					&& (Selectedorder.getDeliveryMethod().equals("Delivery"))) {
				ClientUI.chat.accept("CheckRefund" + "\t" + OrderNumber + "\t" + Selectedorder.getSupplyDate() + "\t"
						+ Selectedorder.getSupplyTime() + "\t" + Selectedorder.getTotalPrice());
				ClientUI.chat.accept(
						"UpdateBalance" + "\t" + ChatClient.refundAmount + "\t" + Selectedorder.getCustomerID());
				if (ChatClient.refundAmount > 0) {
					// show message
					Alert alert = new Alert(AlertType.CONFIRMATION);
					alert.setTitle("Credit updated successfully!");
					alert.setHeaderText("A credit message was sent to the customer in the amount of: "
							+ ChatClient.refundAmount + "NIS");
