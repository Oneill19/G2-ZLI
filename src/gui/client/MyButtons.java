...
/**
 * Class MyButtons() implements Custom-Made by user buttons to simplify code,
 * make it reusable, and shorter.
 * 
 * @author Dorin
 *
 */
public class MyButtons extends Button {
	...
	/**
	 * @return a styled button with purple background for regular clicks with
	 *         setOnMouseEntered styling and setOnMouseExitStyling its' setOnAction
	 *         changes from current fxml to Catalog.fxml
	 */
	public Button createRegularButton() {
		catalogButton = new Button("Catalog",
				new ImageView(new Image(getClass().getResourceAsStream("catalogIcon.png"), 37, 37, false, true)));
		catalogButton.setStyle("-fx-background-color:#a297d5;" + "-fx-background-radius:50;" + "-fx-border-radius:50;"
	...
	class DeleteButtonCell<T, V> extends TableCell<T, V> {
		...
		/**
		 * @param command for special setOnAction.handle() Currently varies by "Cancel
		 *                Order" and "Delete Item" functionalities.
		 */
		DeleteButtonCell(String command) {
			...
			deleteButton.setOnAction(new EventHandler<ActionEvent>() {
				...
				private boolean setAlerts(Order order) {
					...
					case "CANCELED":
						alert.setHeaderText("" + "Order " + order.getOrderNumber() + " was already canceled.");
						alert.showAndWait();
						return false;
					...
					alert.setHeaderText(
							"There are " + Integer.toString(hourdiff) + " hours untill the end of you're supply date.\n"
				...
				public void handle(ActionEvent t) {
					...
					if (command.equals("Cancel Order")) {
						...
						try {
							ClientUI.chat.accept(
									"insertTo_order_cancelation\t" + Integer.toString(selectedO.getOrderNumber()));
						...
						deleteButton.setOnMouseExited(new EventHandler<MouseEvent>() {
							...
							public void handle(MouseEvent e) {
								deleteButton.setStyle("-fx-background-color:green;" + "-fx-background-radius:50;"
										+ "-fx-border-radius:50;" + "-fx-padding: 0 0 0 0;"
					...
					// If button is of deleting an item
						...
						TextField totalPriceField = (TextField) ((Node) t.getTarget()).getScene()
								.lookup("#textFieldPrice");
						TextField totalPriceSale = (TextField) ((Node) t.getTarget()).getScene()
								.lookup("#textFieldSale");
						
						totalPriceField.setEditable(true);
						totalPriceSale.setEditable(true);
						...
						totalPriceSale.setEditable(false);
					}
					...
					// If button is of deActivate sale
						...
						try {
							ClientUI.chat.accept("nullifyIdSaleOfApWithCurrentIdSale\t"
									+ Integer.toString(sale.getIdSale()) + "\t" + "item");
						} catch (Exception e) {
							e.printStackTrace();
						}
						...
						try {
							ClientUI.chat.accept("nullifyIdSaleOfApWithCurrentIdSale\t"
									+ Integer.toString(sale.getIdSale()) + "\t" + "product");
						} catch (Exception e) {
							e.printStackTrace();
						}
	...
	class RegularButtonCell<T, V> extends TableCell<T, V> {
		...
		RegularButtonCell(String command) {
			if (command.equals("Minus"))
				regularBtn = new Button("", new ImageView(
						new Image(getClass().getResourceAsStream("minusAmount2.png"), 30, 30, false, false)));
			else if (command.equals("Plus"))
				regularBtn = new Button("", new ImageView(
						new Image(getClass().getResourceAsStream("plusAmount2.png"), 30, 30, false, false)));

			regularBtn.setStyle("-fx-background-color:#a297d5;" + "-fx-background-radius:50;" + "-fx-border-radius:50;"
					+ "-fx-padding: 0 0 0 0;" + "-fx-border-color:#aeaeae;");
			regularBtn.setPadding(new Insets(5, 0, 0, 5));
			regularBtn.setOnMouseEntered(new EventHandler<MouseEvent>() {
				@Override
				public void handle(MouseEvent e) {
					regularBtn.setStyle("-fx-border-radius:20.0;" + "-fx-background-radius:50;"
							+ "-fx-background-color:#dad5ee;" + "-fx-padding: 0 0 0 0;" + "-fx-border-color:#aeaeae;");
				}
			});
			...
			regularBtn.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent t) {
					if (command.equals("Minus")) {
						AbstractProduct selectedproduct = (AbstractProduct) RegularButtonCell.this.getTableView()
						...
						if (ChatClient.customerCart.get(selectedproduct) == 1) {
							...
							TextField totalPriceField = (TextField) ((Node) t.getTarget()).getScene()
									.lookup("#textFieldPrice");
							TextField totalSaleField = (TextField) ((Node) t.getTarget()).getScene()
									.lookup("#textFieldSale");

							ChatClient.customerCart.remove(selectedproduct); // new due amount
							mytable.getItems().clear();
							mytable.setItems(FXCollections.observableArrayList(ChatClient.customerCart.keySet()));// new
																													// due
							totalPriceField.setEditable(true);
							totalSaleField.setEditable(true);
							// new due amount
	...
	/**
	 * @return a styled button with red background for deletions, with
	 *         setOnMouseEntered styling and setOnMouseExitStyling. It is ment for
	 *         creating buttons that implement TableCell[Order,String]
	 */
	 ...
	 * @return a styled button with red background for deletions, with
	 *         setOnMouseEntered styling and setOnMouseExitStyling. It is ment for
	 *         creating buttons that implement TableCell[AbstractProduct,String]
	 */
	 ...
	 * @return a styled button with red background for deletions, with
	 *         setOnMouseEntered styling and setOnMouseExitStyling. It is ment for
	 *         creating buttons that implement TableCell[Sale,String]
	 */
	...
	/**
	 * @return a styled button with purple background for updates, with
	 *         setOnMouseEntered styling and setOnMouseExitStyling. It is ment for
	 *         creating buttons that implement TableCell[AbstractProduct,String]
	 */
